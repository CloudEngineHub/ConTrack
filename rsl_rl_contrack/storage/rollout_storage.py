...
from rsl_rl_contrack.networks import HiddenState
from rsl_rl_contrack.utils import split_and_pad_trajectories
...
class RolloutStorage:
    ...
    def __init__(
        ...
        reward_dim = 2 if training_type == "rl" else 1
        self.rewards = torch.zeros(num_transitions_per_env, num_envs, reward_dim, device=self.device)
    ...
    def compute_returns(
        ...
        if normalize_advantage:
            self.advantages = (self.advantages - self.advantages.mean(dim=(0, 1), keepdim=True)) / (
                self.advantages.std(dim=(0, 1), keepdim=True) + 1e-8
            )
