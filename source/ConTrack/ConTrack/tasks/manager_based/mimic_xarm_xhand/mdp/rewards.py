...
def tracking_arm(env) -> torch.Tensor:
    ...
    env : isaaclab.envs.ManagerBasedEnv
        Environment instance exposing ``ref_qpos`` (T, H, 19), ``frame_idx`` (num_envs,) and ``ref_hands``.

    Returns
    -------
    torch.Tensor, shape=(num_envs,), dtype=float32
        Mean exp(-mse / 0.5^2) over xArm joints for all hands, where mse is mean squared error in joint units^2.
    """
...
def qvel_penalty_arm(env) -> torch.Tensor:
    ...
    -------
    torch.Tensor, shape=(num_envs,), dtype=float32
        Mean 1 - exp(-mean(v^2) / sigma^2) over xArm joint velocities, where sigma=1.0.
    """
    v = torch.cat(
        [
...
def qacc_penalty_arm(env) -> torch.Tensor:
    ...
    -------
    torch.Tensor, shape=(num_envs,), dtype=float32
        Mean 1 - exp(-mean(a^2) / sigma^2) over xArm joint accelerations, where sigma=50.0.
    """
    v = torch.cat(
        [
            env.scene[name].data.joint_vel[:, joint_ids[:7]]
...
def qacc_penalty_finger(env) -> torch.Tensor:
    ...
    ----------
    env : isaaclab.envs.ManagerBasedEnv
        Environment instance exposing ``prev_finger_qvel`` (num_envs, 12*H), ``ref_hands``, ``cfg.sim.dt`` and
        ``cfg.decimation``.
...
def contact_reward(env) -> torch.Tensor:
    ...
    env : isaaclab.envs.ManagerBasedEnv
        Environment instance exposing:
            ...
            - ``ref_len`` (int)
            - ``ref_hands`` (list[tuple[str, list[int], int]]) with xArm+xHand asset name and reference hand index
            - ``ref_xhand_is_contact`` (T, O, H, 10) bool
            - ``xhand_contact_sensor_keys`` (dict[str, list[str]]) mapping xArm+xHand asset name to 10 ContactSensor keys
            - ``scene`` sensors at those keys with ``data.contact_pos_w`` (num_envs, 1, O, 3) where NaN means no contact

    Returns
    -------
    torch.Tensor, shape=(num_envs,), dtype=float32
        Mean match fraction over hands for the 10 tracked xHand links. Each link contributes 1 if the simulation
    ...
    idx = torch.clamp(env.frame_idx, max=env.ref_len - 1)
    out = []
    for name, _, h in env.ref_hands:
        ref_c = env.ref_xhand_is_contact[idx][:, :, h]
        sim_c = []
...
def contact_distance_reward(env) -> torch.Tensor:
    ...
    env : isaaclab.envs.ManagerBasedEnv
        Environment instance exposing:
            ...
            - ``ref_hands`` (list[tuple[str, list[int], int]]) with xArm+xHand asset name and reference hand index
            - ``ref_xhand_is_contact`` (T, O, H, 10) bool
            - ``ref_xhand_contact_points`` (T, O, H, 10, 3) float32 in object local frame (meters)
            - ``xhand_contact_sensor_keys`` (dict[str, list[str]]) mapping xArm+xHand asset name to 10 ContactSensor keys
    ...
    for name, _, h in env.ref_hands:
        ref_c = env.ref_xhand_is_contact[idx][:, :, h]
        ref_p_local = env.ref_xhand_contact_points[idx][:, :, h]
        ...
        for i, key in enumerate(env.xhand_contact_sensor_keys[name]):
